...
if [ -n "$ERROR_LOG_FILES" ]; then
    for filepath in ${ERROR_LOG_FILES}; do
        if [ -f "$filepath" ]; then
           log YELLOW "Log file: ${filepath}"
           tail -n 10  $filepath
           echo ""
        fi
    done
else
    # timeout으로 인해 ABORTED된 경우, Step 3에서 찾은 recipe의 로그 확인
    if [ -n "${FETCH_RECIPE}" ] && grep -q "Build timed out.*Marking the build as aborted" "${LOG_FILE}"; then
        log YELLOW "Build timed out - checking do_fetch log for ${FETCH_RECIPE}"

        # FETCH_RECIPE는 "libsync-4.1-r0" 형태, 로그에서는 "libsync_4.1.bb" 형태로 검색
        PKG_NAME=$(echo "${FETCH_RECIPE}" | sed 's/-r[0-9]$//' | sed 's/-/_/')

        # Recipe의 .bb 파일 경로를 로그에서 찾기
        RECIPE_BB_PATH=$(grep "${PKG_NAME}\.bb:do_fetch" "${LOG_FILE}" | grep -oE '/data001/[^[:space:]]+\.bb' | head -1)

        if [ -n "${RECIPE_BB_PATH}" ]; then
            # Yocto 로그 파일 경로 찾기: build/tmp*/work/.../temp/log.do_fetch
            BUILD_DIR=${RECIPE_BB_PATH%/poky/*}/build
            LOG_PATH=$(find "${BUILD_DIR}/tmp"*/work -path "*/${PKG_NAME_ONLY}*/temp/log.do_fetch*" 2>/dev/null | head -1)

            if [ -f "${LOG_PATH}" ];
            then log YELLOW "Log: ${LOG_PATH}";  tail -n 10 "${LOG_PATH}" | sed 's/^/  /';  echo ""
            else echo "  Log file not found (build may be already removed)"
            fi
        else
            echo "  Recipe path not found in log"
        fi
    else
        echo "Maybe aborted by System or User"
    fi
fi
