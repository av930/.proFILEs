...
applyTo: "**"
---
<!-- LLM 통제력을 강화하기 위해, 중요한 내용은 영어로 작성 --> 

# GitHub Copilot 커스텀 인스트럭션

## Chat창에 대해 최우선 필수 지침 (AI 사고 과정 노출)
...
3. chat창의 보여주는 모든 코드는 무조건 UTF-8 인코딩으로 작성해줘.
...
```bash
revvserver show              # revv server show 와 동일하며 서버 key값을 tag정보로 grep해서 확인이 가능하다.
```
...
### AI 응답 및 결과 출력 규칙
- 코드 수정 후에는 항상 수정된 내용의 `diff` (변경 사항 부분)를 볼수 있도록 해줘.
