...
## ⚠️ 최우선 필수 지침 (AI 사고 과정 노출)
1. **사고 과정 한글 출력**: 문제를 분석하고 해결책을 찾는 모든 사고(Thinking) 과정과 결과값은 반드시 **한글**로 출력해야 합니다.
2. **숨김(Folding) 방지 및 명시적 출력**: UI에서 기본적으로 접히는(Fold) 내부 동작 로그에 의존하지 마십시오. 사용자가 바로 볼 수 있도록, 답변 메시지 최상단에 항상 일반 텍스트 형태(예: `### 사고 과정`)로 생각과 계획을 펼쳐서(Unfold) 먼저 작성한 후 다른 작업을 진행하십시오.
...
## Terminal 설정
작업중에 실행하는 terminal을 hide시키지 말고 보여줘.
---
